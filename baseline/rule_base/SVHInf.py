...
from utils1 import test_method, search_candidate_mid
# sys.path.append('..')
...
def SHInf(traj_data,aoiwithgrid,grid2aoi,grid_map,node2gps,car_aoi_pinlv):
    ...
    for each_traj_key in total_key_list:
        ...
        for k in range(len(tmp_gps_pair)):
            ...
            if cur_v<3:
                cand_list=search_candidate_mid(cur_tmp_gps_pair,aoiwithgrid,grid2aoi)
        ...
        iiii += 1
    with open("./data/SVHInf/pred_list.json", "w") as f:
        json.dump(total_pred_id, f)
        f.close()
    with open("./data/SVHInf/truth_id.json", "w") as f:
